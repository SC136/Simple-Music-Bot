module.exports = {
    ...
    run: (client, message) => {
        ...
        if (queue.connection.paused) return client.error(message, `The Music Is Already Paused!`);
        const success = queue.setPaused(true);
        if (success) message.reply({ embeds: [client.embed(`\`\`\`Song ${queue.current.title} Paused!\`\`\``)] });
    },
};